...
use std::{
    io::{ErrorKind::NotConnected, Write},
    net::SocketAddr,
    sync::{
        Mutex,
...
pub unsafe extern "C" fn DllMain(_hmodule: usize, reason: u32) -> bool {
    ...
    std::thread::spawn(|| {
        ...
        //                         && stream.peer_addr().err().unwrap().raw_os_error().unwrap()
        //                             != EINPROGRESS
        //                         && stream.peer_addr().err().unwrap().kind() != NotConnected
        //                     {
        //                         socket.lock().unwrap().set_stream(stream.by_ref());
        //                     } else {
        //                         socket.lock().unwrap().clear();
        //                     };
        //                 }
        //             }
        //         }
